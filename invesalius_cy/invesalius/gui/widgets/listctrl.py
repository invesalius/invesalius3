...
#
# 28/11/2008 - Tatiana Al-Chueyr Pereira Martins
# wxTextEditMixin: just works on column 1
...
class ColumnSorterMixin:
    ...
    def __OnColClick(self, evt):
        ...
        if (
            ...
            or wx.SystemOptions.GetOptionInt("mac.listctrl.always_use_generic") == 1
        ):
            self.__updateImages(oldCol)
        evt.Skip()
...
class TextEditMixin:
    ...
    def OnLeftDown2(self, evt=None):
        ...
        if flags == wx.LIST_HITTEST_ONITEMICON:
            ...
            begin_index = index
            end_index = index
            if self.__last_check_ is not None and wx.GetKeyState(wx.WXK_SHIFT):
