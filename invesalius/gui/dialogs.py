...
class MaskEvent(wx.PyCommandEvent):
    def __init__(self , evtType, id, mask_index) -> None:
...
class NumberDialog(wx.Dialog):
    def __init__(self, message, value=0) -> None:
        ...
        self.num_ctrl: NumCtrl = num_ctrl
...
class ResizeImageDialog(wx.Dialog):
    ...
    def __init__(self) -> None:#, message, value=0):
        wx.Dialog.__init__(self, None, -1, "InVesalius 3", size=wx.DefaultSize,
                          pos=wx.DefaultPosition,
                          style=wx.DEFAULT_DIALOG_STYLE)
        ...
        self.num_ctrl_porcent: InvSpinCtrl = num_ctrl_percent
    ...
    def GetValue(self)-> (int | Any):
        return self.num_ctrl_porcent.GetValue()

    def Close(self) -> None:
        self.Destroy()

def ShowNumberDialog(message, value=0)-> Any | Literal[0]:
...
def ShowOpenProjectDialog()-> (Any | None):
    # Default system path
...
def ShowImportDirDialog(self)-> (Any | None):
    current_dir: str = os.path.abspath(".")

    if sys.platform == 'win32' or sys.platform.startswith('linux'):
        session = ses.Session()
        folder: Any | Literal[''] = session.GetConfig('last_dicom_folder', '')
    else:
        folder = ''
...
def ShowImportBitmapDirDialog(self)-> (Any | None):
    current_dir: str = os.path.abspath(".")
...
def ShowImportOtherFilesDialog(id_type, msg='Import NIFTi 1 file')-> (Any | None):
    # Default system path
    session = ses.Session()
    last_directory:Any | Literal[''] = session.GetConfig('last_directory_%d' % id_type, '')
...
def ShowImportMeshFilesDialog() -> (Any | None):
    # Default system path
    current_dir: str = os.path.abspath(".")

    session = ses.Session()
    last_directory: Any | Literal[''] = session.GetConfig('last_directory_surface_import', '')
...
def ShowSaveAsProjectDialog(default_filename=None)-> tuple[Any | None, Any]:
    current_dir: str = os.path.abspath(".")

    session = ses.Session()
    last_directory: Any | Literal[''] = session.GetConfig('last_directory_inv3', '')
...
def ShowLoadCSVDebugEfield(message=_(u"Load debug CSV Enorm file"), current_dir=os.path.abspath("."), style=wx.FD_OPEN | wx.FD_CHANGE_DIR,
                           wildcard=_("(*.csv)|*.csv"), default_filename="") -> ndarray[Any, dtype] | None:
    ...
    if filepath:
        with open(filepath, 'r') as file:
            my_reader: _reader = csv.reader(file, delimiter=',')
            rows = []
            for row in my_reader:
                rows.append(row)
        e_field: list = rows
        e_field_norms: ndarray[Any, dtype] = np.array(e_field).astype(float)
...
def ShowLoadSaveDialog(message=_(u"Load File"), current_dir=os.path.abspath("."), style=wx.FD_OPEN | wx.FD_CHANGE_DIR,
                       wildcard=_("Registration files (*.obr)|*.obr"), default_filename="", save_ext=None) -> None:
...
class MessageBox(wx.Dialog):
    def __init__(self, parent, title, message, caption="InVesalius3 Error") -> None:
...
def SaveChangesDialog__Old(filename) -> Literal[1, 0, -1]:
...
def ImportEmptyDirectory(dirpath) -> None:
...
def ImportInvalidFiles(ftype="DICOM") -> None:
    if ftype == "Bitmap":
...
def ShowNavigationTrackerWarning(trck_id, lib_mode) -> None:
    """
    Spatial Tracker connection error
    """
    trck: dict[int, str] = {const.SELECT: 'Tracker',
...
def ICPcorregistration(fre) -> bool:
...
def ReportICPerror(prev_error, final_error) -> None:
...
def ShowColorDialog(color_current)-> (Any | None):
    cdata = wx.ColourData()
...
class NewMask(wx.Dialog):
    def __init__(self,
        ...
        default_index: int = thresh_list.index(_("Bone"))
        ...
        flag_link: int = wx.EXPAND|wx.GROW|wx.ALL
        flag_button: int = wx.ALL | wx.EXPAND| wx.GROW
    ...
    def OnSlideChanged(self, evt) -> None:
        import invesalius.project as prj
        thresh_min: int = self.gradient.GetMinValue()
        thresh_max: int = self.gradient.GetMaxValue()
        thresh: tuple[int, int] = (thresh_min, thresh_max)
        proj = prj.Project()
        if thresh  in proj.threshold_modes.values():
            preset_name = proj.threshold_modes.get_key(thresh)[0]
            index: int = self.thresh_list.index(preset_name)
            self.combo_thresh.SetSelection(index)
        else:
            index: int = self.thresh_list.index(_("Custom"))
            self.combo_thresh.SetSelection(index)

    def GetValue(self)-> tuple[Any, list[int], Any]:
        #mask_index = self.combo_mask.GetSelection()
        mask_name = self.text.GetValue()
        thresh_value: list[int] = [self.gradient.GetMinValue(), self.gradient.GetMaxValue()]
...
def SaveChangesDialog(filename, parent) -> Literal[1, 0, -1]:
    current_dir: str = os.path.abspath(".")
...
def SaveChangesDialog2(filename) -> Literal[1, 0]:
    current_dir: str = os.path.abspath(".")
...
def ShowSavePresetDialog(default_filename="raycasting")-> (Any | None):
...
class NewSurfaceDialog(wx.Dialog):
    def __init__(self, parent=None, ID=-1, title="InVesalius 3", size=wx.DefaultSize,
        ...
        self.combo_quality: ComboBox = combo_quality
        ...
        self.check_box_holes: CheckBox = check_box_holes
        check_box_largest = wx.CheckBox(self, -1, _("Keep largest region"))
        self.check_box_largest: CheckBox = check_box_largest
...
def ExportPicture(type_="")-> (tuple | tuple[()]):
    import invesalius.constants as const
    import invesalius.project as proj

    INDEX_TO_EXTENSION: dict[int, str] = {0: "bmp", 1: "jpg", 2: "png", 3: "ps", 4:"povray", 5:"tiff"}
    ...
    INDEX_TO_TYPE: dict[int, Any] = {0: const.FILETYPE_BMP,
    ...
    session = ses.Session()
    last_directory: Any | Literal[''] = session.GetConfig('last_directory_screenshot', '')

    project_name: str = "%s_%s" % (project.name, type_)
...
class SurfaceCreationDialog(wx.Dialog):
    ...
    def GetValue(self)-> dict[str, dict[str, Any]]:
...
class SurfaceCreationOptionsPanel(wx.Panel):
    def __init__(self, parent, ID=-1) -> None:
        ...
        #Retrieve existing masks
        project = prj.Project()
        index_list: dict_keys = project.mask_dict.keys()
        ...
        self.check_box_border_holes: CheckBox = check_box_border_holes
        check_box_holes = wx.CheckBox(self, -1, _("Fill holes"))
...
class SurfaceMethodPanel(wx.Panel):
    ...
    `Context aware smoothing'
    '''
    def __init__(self, parent, id, mask_edited=False) -> None:
        wx.Panel.__init__(self, parent, id)

        self.mask_edited: bool = mask_edited
        self.alg_types: dict[Any, str] = {_(u'Default'): 'Default',
    ...
    def _set_cb_types(self, evt) -> None:
    ...
    def GetOptions(self)-> dict[str, Any]:
        if self.GetAlgorithmSelected() == 'ca_smoothing':
            options: dict[str, Any] = {'angle': self.ca_options.angle.GetValue(),
    ...
    def GetValue(self)-> dict[str, Any]:
        algorithm: str = self.GetAlgorithmSelected()
        options: dict[str, Any] = self.GetOptions()
...
class ClutImagedataDialog(wx.Dialog):
    ...
    def Show(self, gen_evt=True, show=True) -> None:
...
class WatershedOptionsPanel(wx.Panel):
    def __init__(self, parent, config) -> None:
        ...
        self.algorithms: tuple[Literal['Watershed'], Literal['Watershed IFT']] = ("Watershed", "Watershed IFT")
        self.con2d_choices: tuple[Literal[4], Literal[8]] = (4, 8)
        self.con3d_choices: tuple[Literal[6], Literal[18], Literal[26]] = (6, 18, 26)
...
class WatershedOptionsDialog(wx.Dialog):
    ...
    def _init_gui(self) -> None:
        ...
        self.wop: WatershedOptionsPanel = wop
...
class MaskBooleanDialog(wx.Dialog):
    ...
    def _init_gui(self, masks) -> None:
        mask_choices: list[tuple] = [(masks[i].name, masks[i]) for i in sorted(masks)]
        ...
        icon_folder: Path = inv_paths.ICON_DIR
        op_choices: tuple[tuple[Any, Literal[1], Literal['bool_union.png']], tuple[Any, Literal[2], Literal['bool_difference.png']], tuple[Any, Literal[3], Literal['bool_intersection.png']], tuple[Any, Literal[4], Literal['bool_disjunction.png']]] = ((_(u"Union"), const.BOOLEAN_UNION, 'bool_union.png'),
...
class ReorientImageDialog(wx.Dialog):
    ...
    def _init_gui(self) -> None:
        interp_methods_choices: tuple[tuple[Any, Literal[0]], tuple[Any, Literal[1]], tuple[Any, Literal[2]], tuple[Any, Literal[3]]] = ((_(u"Nearest Neighbour"), 0),
                                  (_(u"Trilinear"), 1),
                                  (_(u"Tricubic"), 2),
    ...
    def _update_angles(self, angles) -> None:
    ...
    def apply_reorientation(self, evt) -> None:
...
class ImportBitmapParameters(wx.Dialog):
    ...
    def _init_gui(self) -> None:
        ...
        flag_labels: int = wx.ALIGN_RIGHT  | wx.ALIGN_CENTER_VERTICAL
...
class PanelTargeFFill(wx.Panel):
    def __init__(self, parent, ID=-1, style=wx.TAB_TRAVERSAL|wx.NO_BORDER) -> None:
...
class Panel2DConnectivity(wx.Panel):
    def __init__(self, parent, ID=-1, show_orientation=False, style=wx.TAB_TRAVERSAL|wx.NO_BORDER) -> None:
        wx.Panel.__init__(self, parent, ID, style=style)
        self._init_gui(show_orientation)

    def _init_gui(self, show_orientation) -> None:
...
class PanelFFillThreshold(wx.Panel):
    def __init__(self, parent, config, ID=-1, style=wx.TAB_TRAVERSAL|wx.NO_BORDER) -> None:
        wx.Panel.__init__(self, parent, ID, style=style)
...
class PanelFFillDynamic(wx.Panel):
    ...
    def OnSetUseWWWL(self, evt) -> None:
        self.config.use_ww_wl = self.use_ww_wl.GetValue()

    def OnSetDeviation(self, evt) -> None:
...
class PanelFFillConfidence(wx.Panel):
    ...
    def OnSetMult(self, evt) -> None:
        self.config.confid_mult = self.spin_mult.GetValue()

    def OnSetIters(self, evt) -> None:
...
class FFillOptionsDialog(wx.Dialog):
    ...
    def _init_gui(self) -> None:
        ...
        # Target
        if sys.platform == "win32":
            border_style: Literal[0] = wx.SIMPLE_BORDER
        else:
...
class CropOptionsDialog(wx.Dialog):
    ...
    def UpdateValues(self, limits) -> None:
    ...
    def _init_gui(self) -> None:
        ...
        txt_style: Literal[0] = wx.TE_READONLY
...
class FillHolesAutoDialog(wx.Dialog):
    ...
    def OnApply(self, evt) -> None:
        if self.panel_target.target_2d.GetValue():
            ...
            orientation: str = self.panel2dcon.GetOrientation()
        else:
            target = "3D"
...
class MaskDensityDialog(wx.Dialog):
    ...
    def _init_gui(self) -> None:
        ...
        self.mean_density: TextCtrl = self._create_selectable_label_text('')
        self.min_density: TextCtrl = self._create_selectable_label_text('')
        self.max_density: TextCtrl = self._create_selectable_label_text('')
        self.std_density: TextCtrl = self._create_selectable_label_text('')
    ...
    def _create_selectable_label_text(self, text) -> TextCtrl:
    ...
    def OnCalcButton(self, evt) -> None:
        ...
        with futures.ThreadPoolExecutor(max_workers=1) as executor:
            future: Future[tuple[float, float, float, float]] = executor.submit(slc.calc_image_density, mask)
...
class ObjectCalibrationDialog(wx.Dialog):
    ...
    def __init__(self, tracker, pedal_connection, neuronavigation_api) -> None:
        ...
        self.obj_fiducials: ndarray[Any, dtype] = np.full([4, 3], np.nan)
        self.obj_orients: ndarray[Any, dtype] = np.full([4, 3], np.nan)
    ...
    def _init_gui(self) -> None:
        ...
        self.btns_coord: list[None] = [None] * 4
        self.text_actors: list[None] = [None] * 4
        self.ball_actors: list[None] = [None] * 4
        ...
        self.choice_sensor: ComboBox = choice_sensor
    ...
    def ObjectImportDialog(self) -> Literal[1, 0]:
    ...
    def ShowObject(self, polydata) -> None:
    ...
    def ConfigureObject(self) -> bool:
        ...
        if use_default_object:
            path: str = os.path.join(inv_paths.OBJ_DIR, "magstim_fig8_coil.stl")
            ...
            valid_extensions: tuple[Literal['.stl'], Literal['ply'], Literal['.obj'], Literal['.vtp']] = ('.stl', 'ply', '.obj', '.vtp')
        ...
        self.obj_name: bytes = path.encode(const.FS_ENCODE)
        self.use_default_object = use_default_object

        return True

    def InitializeObject(self) -> None:
        success: bool = self.ConfigureObject()
        if success:
            ...
            #
            object_path: str = self.obj_name.decode(const.FS_ENCODE)
    ...
    def OnCreateObjectText(self, name, coord) -> tuple[vtkActor, vtkFollower]:
        ...
        tactor.SetScale(5)
        ball_position: Tuple[float, float, float] = ball_actor.GetPosition()
    ...
    def OnObjectFiducialButton(self, index, evt, ctrl) -> None:
        ...
        #
        def set_fiducial_callback(state) -> None:
            if state:
    ...
    def SetObjectFiducial(self, fiducial_index) -> None:
    ...
    def GetValue(self)-> tuple[Any, Any, int, bytes, Any | None, bool | Any]:
...
class ICPCorregistrationDialog(wx.Dialog):
    ...
    def __init__(self, navigation, tracker) -> None:
        import invesalius.project as prj
        ...
        self.obj_fiducials: ndarray[Any, dtype] = np.full([5, 3], np.nan)
        self.obj_orients: ndarray[Any, dtype] = np.full([5, 3], np.nan)
    ...
    def _init_gui(self) -> None:
        ...
        self.cont_point: ToggleButton = cont_point
    ...
    def LoadActor(self) -> None:
        ...
        obj_actor = vtkActor()
        obj_actor.SetMapper(mapper)
        self.obj_actor: vtkActor = obj_actor
        ...
        self.collect_points: Text = collect_points
        ...
        self.txt_markers_not_detected: vtkActor2D = txt_markers_not_detected.actor
    ...
    def GetCurrentCoord(self) -> Tuple[np.ndarray, bool]:
    ...
    def AddMarker(self, size, colour, coord) -> None:
        """
    ...
    def vtkmatrix_to_numpy(self, matrix) -> ndarray[Any, dtype[floating[_64Bit]]]:
        """
        ...
        :rtype: numpy.ndarray
        """
        m: ndarray[Any, dtype[floating[_64Bit]]] = np.ones((4, 4))
        for i in range(4):
            for j in range(4):
                m[i, j] = matrix.GetElement(i, j)
        return m

    def SetCameraVolume(self, position) -> None:
        """
        ...
        if self.initial_focus is None:
            self.initial_focus: ndarray[Any, dtype] = np.array(cam.GetFocalPoint())

        cam_pos0: ndarray[Any, dtype] = np.array(cam.GetPosition())
    ...
    def CheckTransformedPointsDistribution(self, points) -> floating:
        from scipy.spatial.distance import pdist
        return np.mean(pdist(points))

    def ErrorEstimation(self, surface, points) -> floating:
        """
    ...
    def OnContinuousAcquisitionButton(self, evt=None, btn=None) -> None:
        value = btn.GetValue()
        if value:
    ...
    def HandleContinuousAcquisition(self, evt) -> None:
        self.CreatePoint()
    ...
    def GetValue(self)-> tuple[Any | None, list, list, Any | None, Any | None]:
...
class SetCoilOrientationDialog(wx.Dialog):
    ...
    def __init__(self, marker, mTMS=None, brain_target=False, brain_actor=None) -> None:
        import invesalius.project as prj
        ...
        self.mTMS = mTMS
        self.marker = marker
        self.brain_target: bool = brain_target
    ...
    def _init_gui(self) -> None:
        ...
        self.obj_actor: vtkActor = self.LoadActor(self.surface)
        if self.brain_target:
            self.brain_surface = self.surface
        else:
            self.brain_actor: vtkActor = self.LoadActor(self.brain_surface)
        self.coil_pose_actor: vtkActor = self.LoadTarget()
        ...
        self.slider_rotation_x: Slider = slider_rotation_x
        ...
        self.slider_rotation_y: Slider = slider_rotation_y
        ...
        self.slider_rotation_z: Slider = slider_rotation_z
        ...
        flag_link: int = wx.EXPAND|wx.GROW|wx.RIGHT|wx.TOP
        flag_slider: int = wx.EXPAND|wx.GROW|wx.LEFT
    ...
    def OnCreateDummyCoil(self, target_actor) -> None:
        if self.dummy_coil_actor:
            self.RemoveActor(self.dummy_coil_actor)

        filename: str = os.path.join(inv_paths.OBJ_DIR, "magstim_fig8_coil_no_handle.stl")
    ...
    def OnWheelMouseClick(self, obj, evt) -> None:
        ...
        if self.picker.GetActor():
            if self.brain_target:
                colour: list[int] = [1, 0, 0]
            else:
                colour: list[int] = [0, 0, 1]
            self.marker_actor.GetProperty().SetColor(colour)
            self.marker_actor: vtkActor = self.picker.GetActor()
    ...
    def OnRotationX(self, evt) -> None:
        self.rotationX = evt.GetInt()
        self.marker_actor.SetOrientation(self.rotationX, self.rotationY, self.rotationZ)
        self.interactor.Render()

    def OnRotationY(self, evt) -> None:
        self.rotationY = evt.GetInt()
        self.marker_actor.SetOrientation(self.rotationX, self.rotationY, self.rotationZ)
        self.interactor.Render()

    def OnRotationZ(self, evt) -> None:
        self.rotationZ = evt.GetInt()
        self.marker_actor.SetOrientation(self.rotationX, self.rotationY, self.rotationZ)
        self.interactor.Render()

    def OnDepth(self, evt) -> None:
    ...
    def LoadCenterBrainTarget(self, coil_target_position, coil_target_orientation) -> None:
    ...
    def LoadActor(self, surface) -> vtkActor:
        '''
        Load the selected actor from the project (self.surface) into the scene
        :return:
        '''
    ...
    def AddTarget(self, coord_flip, colour=[0.0, 0.0, 1.0], scale=10)-> Tuple[vtkActor, List[float]]:
        ...
        marker_actor: vtkActor = self.CreateActorArrow(m_img_vtk, colour=colour)
    ...
    def CreateActorArrow(self, m_img_vtk, colour, size=const.ARROW_MARKER_SIZE) -> vtkActor:
        input1 = vtkPolyData()
        input2 = vtkPolyData()
        input3 = vtkPolyData()
        input4 = vtkPolyData()
    ...
    def vtkmatrix2numpy(self, matrix) -> ndarray[Any, dtype[floating[_64Bit]]]:
        """
        Copies the elements of a vtkMatrix4x4 into a numpy array.
        param matrix: The matrix to be copied into an array.
        Args:
    ...
    def ICP(self, coord, center, surface) -> tuple[int, int, int, None, None, None]:
        """
        ...
        self.m_icp: ndarray[Any, dtype[floating[_64Bit]]] = self.vtkmatrix2numpy(icp.GetMatrix())
        ...
        coord: tuple[int, int, int, None, None, None] = p[0], p[1], p[2], None,  None,  None
    ...
    def CreateGrid(self, resolution, space_x, space_y) -> list[ndarray[Any, dtype]]:
    ...
    def OnCreateRandomTargetGrid(self, evt) -> None:
        ...
        narray: ndarray[Any, dtype[floating[_64Bit]]] = np.eye(4)
        ...
        if self.center_brain_target_actor is None:
            self.center_brain_target_actor: None = self.LoadCenterBrainTarget(coil_target_position, coil_target_orientation)
        ...
        for i in range(number_of_targets):
            alpha: float = 2 * np.pi * random.random()
            r = circle_r * np.sqrt(random.random())
            X = r * np.cos(alpha)
            Y = r * np.sin(alpha)
            rZ: int = random.randrange(-30, 30, 1)
    ...
    def OnCreateTargetGrid(self, evt) -> None:
        ...
        narray: ndarray[Any, dtype[floating[_64Bit]]] = np.eye(4)
        vtkmat.DeepCopy(narray.ravel(), vtkmat)
        position: list = [narray[0][-1], narray[1][-1], narray[2][-1]]
        m_rotation = [narray[0][:3], narray[1][:3], narray[2][:3]]
        coil_target_position = position
        coil_target_orientation = np.rad2deg(tr.euler_from_matrix(m_rotation, axes="sxyz"))
        ...
        for i in range(grid_resolution):
            for j in range(grid_resolution):
                ...
                coord_scalp: tuple[int, int, int, None, None, None] = self.ICP(position, coil_target_position, self.surface)
                coord: tuple[int, int, int, Any, Any, Any] = coord_scalp[0], coord_scalp[1], coord_scalp[2], coil_target_orientation[0], \
    ...
    def OnSendMtms(self, evt=None) -> None:
        ...
        vtkmat.DeepCopy(narray.ravel(), vtkmat)
        position: list = [narray[0][-1], -narray[1][-1], narray[2][-1]]
        m_rotation: list = [narray[0][:3], narray[1][:3], narray[2][:3]]
    ...
    def CreateVTKObjectMatrix(self, direction, orientation, new_target)-> tuple[Any, float, float, float]:
    ...
    def SetVolumeCamera(self, cam_focus) -> None:
        ...
        cam_pos0: ndarray[Any, dtype] = np.array(cam.GetPosition())
        cam_focus0: ndarray[Any, dtype] = np.array(cam.GetFocalPoint())
        v0 = cam_pos0 - cam_focus0
        v0n = np.sqrt(inner1d(v0, v0))
    ...
    def GetRotationMatrix(self, v1_start, v2_start, v1_target, v2_target):
        ...
        u3_start: ndarray[Any, dtype[floating]] = self.Normalize(np.cross(u1_start, u2_start))

        u1_target = self.Normalize(v1_target)
        u2_target = self.Normalize(v2_target)
        u3_target: ndarray[Any, dtype[floating]] = self.Normalize(np.cross(u1_target, u2_target))
    ...
    def GetEulerAnglesFromVectors(self, init_arrow_vector, target_arrow_vector) -> ndarray[Any, dtype]:
        import invesalius.data.transformations as tr
    ...
    def CenterOfMass(self, surface) -> list[float]:
    ...
    def GetValue(self)-> tuple[list[list], list[NDArray], list, list]:
        ...
        m_rotation = [narray[0][:3], narray[1][:3], narray[2][:3]]
        coil_target_position = [position]
        coil_target_orientation: list[ndarray[Any, dtype]] = [np.rad2deg(tr.euler_from_matrix(m_rotation, axes="sxyz"))]
    ...
    def GetValueBrainTarget(self)-> tuple[list, list]:
        import invesalius.data.transformations as tr
        brain_target_position = []
        brain_target_orientation = []
        for brain_target_actor in self.brain_target_actor_list:
...
class SurfaceProgressWindow(object):
    def __init__(self) -> None:
        ...
        self.style: int = wx.PD_APP_MODAL | wx.PD_APP_MODAL | wx.PD_CAN_ABORT | wx.PD_ELAPSED_TIME
    ...
    def Update(self, msg=None, value=None) -> None:
        if msg is None:
            self.dlg.Pulse()
        else:
            self.dlg.Pulse(msg)
...
class GoToDialog(wx.Dialog):
    def __init__(self, title=_("Go to slice ..."), init_orientation=const.AXIAL_STR) -> None:
        wx.Dialog.__init__(self, wx.GetApp().GetTopWindow(), -1, title, style=wx.DEFAULT_DIALOG_STYLE|wx.FRAME_FLOAT_ON_PARENT)
        self._init_gui(init_orientation)

    def _init_gui(self, init_orientation) -> None:
        orientations = (
    ...
    def SetNewFocalPoint(self, coord, spacing) -> None:
        newCoord = list(coord)
...
class GoToDialogScannerCoord(wx.Dialog):
    def __init__(self, title=_("Go to scanner coord...")) -> None:
    ...
    def _init_gui(self) -> None:
        ...
        self.orientation = None
        self.affine: ndarray[Any, dtype[floating[_64Bit]]] = np.identity(4)
    ...
    def OnOk(self, evt) -> None:
        import invesalius.data.slice_ as slc
        try:
            point: list[float] = [float(self.goto_sagital.GetValue()),
...
class SetOptitrackconfigs(wx.Dialog):
    def __init__(self, title=_("Setting Optitrack configs:")) -> None:
    ...
    def _init_gui(self) -> None:
        ...
        if not last_optitrack_cal_dir:
            last_optitrack_cal_dir: str = inv_paths.OPTITRACK_CAL_DIR
        if not last_optitrack_User_Profile_dir:
            last_optitrack_User_Profile_dir: str = inv_paths.OPTITRACK_USERPROFILE_DIR
...
class SetTrackerDeviceToRobot(wx.Dialog):
    ...
    """
    def __init__(self, title=_("Setting tracker device:")) -> None:
        wx.Dialog.__init__(self, wx.GetApp().GetTopWindow(), -1, title, size=wx.Size(1000, 200),
                           style=wx.DEFAULT_DIALOG_STYLE|wx.FRAME_FLOAT_ON_PARENT|wx.STAY_ON_TOP|wx.RESIZE_BORDER)
    ...
    def OnChoiceTracker(self, evt, ctrl) -> None:
...
class SetRobotIP(wx.Dialog):
    def __init__(self, title=_("Setting Robot IP")) -> None:
        wx.Dialog.__init__(self, wx.GetApp().GetTopWindow(), -1, title, size=wx.Size(1000, 200),
                           style=wx.DEFAULT_DIALOG_STYLE|wx.FRAME_FLOAT_ON_PARENT|wx.STAY_ON_TOP|wx.RESIZE_BORDER)
        self.robot_ip = None
        self._init_gui()
    ...
    def OnTxt_Ent(self, evt, ctrl) -> None:
        self.robot_ip = str(ctrl.GetValue())

    def OnChoiceIP(self, evt, ctrl) -> None:
        self.robot_ip = ctrl.GetStringSelection()

    def GetValue(self)-> (str | Any | None):
        return self.robot_ip

class RobotCoregistrationDialog(wx.Dialog):
    def __init__(self, tracker, title=_("Create transformation matrix to robot space")) -> None:
        wx.Dialog.__init__(self, wx.GetApp().GetTopWindow(), -1, title, #size=wx.Size(1000, 200),
                           style=wx.DEFAULT_DIALOG_STYLE|wx.FRAME_FLOAT_ON_PARENT|wx.STAY_ON_TOP|wx.RESIZE_BORDER)
        '''
    ...
    def _init_gui(self) -> None:
        ...
        self.btn_apply_reg: Button = btn_apply_reg
    ...
    def UpdateRobotConnectionStatus(self, data) -> None:
    ...
    def ApplyRegistration(self, evt) -> None:
        self.StopContinuousAcquisition()
    ...
    def UpdateRobotTransformationMatrix(self, data) -> None:
        self.matrix_tracker_to_robot = np.array(data)

    def SaveRegistration(self, evt) -> None:
        if self.matrix_tracker_to_robot is None:
            return

        # Open dialog to choose filename.
        filename: None = ShowLoadSaveDialog(
        ...
        with open(filename, 'w', newline='') as file:
            writer: _writer = csv.writer(file, delimiter='\t')
            writer.writerows(np.vstack(self.matrix_tracker_to_robot).tolist())

    def LoadRegistration(self, evt) -> None:
        # Open dialog to choose filename.
        filename: None = ShowLoadSaveDialog(
        ...
        with open(filename, 'r') as file:
            reader: _reader = csv.reader(file, delimiter='\t')
            content: list[list[str]] = [row for row in reader]
    ...
    def GetValue(self)-> (NDArray | Any | list):
        return self.matrix_tracker_to_robot


class SetNDIconfigs(wx.Dialog):
    def __init__(self, title=_("Setting NDI polaris configs:")) -> None:
        wx.Dialog.__init__(self, wx.GetApp().GetTopWindow(), -1, title, size=wx.Size(1000, 200),
                           style=wx.DEFAULT_DIALOG_STYLE|wx.FRAME_FLOAT_ON_PARENT|wx.STAY_ON_TOP|wx.RESIZE_BORDER)
        self._init_gui()

    def serial_ports(self) -> tuple[list, list[int]]:
        """
        ...
        if sys.platform.startswith('win'):
            ...
            port_selec: list[int] = [i for i, e in enumerate(desc_list) if 'NDI' in e]
        else:
            for p in ports:
    ...
    def _init_gui(self) -> None:
        ...
        self.com_ports: ComboBox = com_ports
        ...
        if not last_ndi_probe_marker:
            last_ndi_probe_marker: str = inv_paths.NDI_MAR_DIR_PROBE
        if not last_ndi_ref_marker:
            last_ndi_ref_marker: str = inv_paths.NDI_MAR_DIR_REF
        if not last_ndi_obj_marker:
            last_ndi_obj_marker: str = inv_paths.NDI_MAR_DIR_OBJ
    ...
    def OnChoicePort(self, evt, ctrl) -> None:
        self.btn_ok.Enable(True)
...
class SetCOMPort(wx.Dialog):
    def __init__(self, select_baud_rate, title=_("Select COM port")) -> None:
        wx.Dialog.__init__(self, wx.GetApp().GetTopWindow(), -1, title, style=wx.DEFAULT_DIALOG_STYLE | wx.FRAME_FLOAT_ON_PARENT | wx.STAY_ON_TOP)
    ...
    def serial_ports(self) -> list[str]:
        ...
        import serial.tools.list_ports
        if sys.platform.startswith('win'):
            ports: list[str] = ([comport.device for comport in serial.tools.list_ports.comports()])
        else:
    ...
    def _init_gui(self) -> None:
        # COM port selection
        ports: list[str] = self.serial_ports()
        ...
        if self.select_baud_rate:
            baud_rates_as_strings: list[str] = [str(baud_rate) for baud_rate in const.BAUD_RATES]
...
class SetSpacingDialog(wx.Dialog):
    ...
    def OnSetNewSpacing(self, evt) -> None:
        try:
    ...
    def set_new_spacing(self, sx, sy, sz) -> None:
...
class PeelsCreationDlg(wx.Dialog):
    ...
    def _init_gui(self) -> None:
        ...
        from_mask_stbox: StaticBoxSizer = self._from_mask_gui()
        from_files_stbox: StaticBoxSizer = self._from_files_gui()
    ...
    def _from_mask_gui(self) -> StaticBoxSizer:
    ...
    def _from_files_gui(self) -> StaticBoxSizer:
        session = ses.Session()
    ...
    def _set_files_callback(self, mask_path='') -> None:
        if mask_path:
