...
class MarkersPanel(wx.Panel):
    ...
    def OnMouseRightDown(self, evt):
        ...
        brain_orientation_menu = menu_id.Append(4, _('Set brain target orientation'))
        menu_id.Bind(wx.EVT_MENU, self.OnMenuSetBrainOrientation, brain_orientation_menu)
        ...
        if self.tracker.tracker_id == const.ROBOT:
            send_target_to_robot_compensation = menu_id.Append(5, _('Sets target to robot head move compensation'))
            menu_id.Bind(wx.EVT_MENU, self.OnMenuSetRobotCompensation, send_target_to_robot_compensation)
            send_target_to_robot = menu_id.Append(6, _('Send target from InVesalius to robot'))
    ...
    def OnMenuSetBrainOrientation(self, evt):
        list_index = self.lc.GetFocusedItem()
        marker = self.markers[list_index].coord
        dialog = dlg.SetBrainOrientationDialog(marker=marker)
