...
def img2memmap(group):
    ...
    print(
        f"[NIfTI Import] Raw data from file: dtype={data.dtype}, "
    ...
    if np.ptp(data) > (2**16 / 2 - 1):
        print(
            f"[NIfTI Import] Rescaling: ptp={np.ptp(data):.4f} > {2**16 / 2 - 1}, normalizing to 0-10000"
        )
    ...
    # have to be binary (0, 1). To prevent that, rescale pixel values from 0-255
        ...
        print(
            f"[NIfTI Import] Small values detected: max={data.max():.4f} < 8, offering rescale dialog"
        )
    ...
    print(
        f"[NIfTI Import] Final memmap: dtype={matrix.dtype}, "
