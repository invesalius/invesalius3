...
from vtkmodules.vtkImagingCore import vtkImageActor, vtkImageCast, vtkImageReslice
...
BRUSH_FOREGROUND: int = 1
BRUSH_BACKGROUND: int = 2
BRUSH_ERASE: int = 0

WATERSHED_OPERATIONS: dict = {_("Erase"): BRUSH_ERASE,
...
class BaseImageInteractorStyle(vtkInteractorStyleImage):
    ...
    def OnPressLeftButton(self, evt: Any, obj: Any) -> None:
        self.left_pressed = True

    def OnReleaseLeftButton(self, evt: Any, obj: Any) -> None:
        self.left_pressed = False

    def OnPressRightButton(self, evt: Any, obj: Any) -> None:
        self.right_pressed = True
        self.viewer.last_position_mouse_move = \
            self.viewer.interactor.GetLastEventPosition()

    def OnReleaseRightButton(self, evt: Any, obj: Any) -> None:
        self.right_pressed = False

    def OnMiddleButtonPressEvent(self, evt: Any, obj: Any) -> None:
        self.middle_pressed = True

    def OnMiddleButtonReleaseEvent(self, evt: Any, obj: Any) -> None:
    ...
    def GetPickPosition(self, mouse_position: Optional[Tuple[int, int]] = None) -> Tuple[float, float, float]:
...
class DefaultInteractorStyle(BaseImageInteractorStyle):
    ...
    * Change the slices with the scroll.
    ...
    def OnFocus(self, evt: Any, obj: Any) -> None:
        self.viewer.SetFocus()

    def OnZoomRightMove(self, evt: Any, obj: Any) -> None:
    ...
    def OnZoomRightClick(self, evt: Any, obj: Any) -> None:
        evt.StartDolly()

    def OnZoomRightRelease(self, evt: Any, obj: Any) -> None:
        print('EndDolly')
        evt.OnRightButtonUp()
        #  evt.EndDolly()
        self.right_pressed = False

    def OnScrollForward(self, evt: Any, obj: Any) -> None:
        iren = self.viewer.interactor
        viewer = self.viewer
        if  iren.GetShiftKey():
    ...
    def OnScrollBackward(self, evt: Any, obj: Any) -> None:
        iren = self.viewer.interactor
        viewer = self.viewer
...
class BaseImageEditionInteractorStyle(DefaultInteractorStyle):
    def __init__(self, viewer: Any) -> None:
        ...
        self.viewer: Any = viewer
        self.orientation: str = self.viewer.orientation

        self.picker: vtkWorldPointPicker = vtkWorldPointPicker()
        self.matrix: Optional[np.ndarray] = None

        self.cursor: Optional[Union[ca.CursorRectangle, ca.CursorCircle]] = None
        self.brush_size: int = const.BRUSH_SIZE
        self.brush_format: str = const.DEFAULT_BRUSH_FORMAT
        self.brush_colour: Tuple[int, int, int] = const.BRUSH_COLOUR
        self._set_cursor()

        self.fill_value: int = 254
    ...
    def set_brush_format(self, format: str) -> None:
        self.brush_format = format
        self._set_cursor()

    def set_brush_operation(self, operation: Any) -> None:
        self.brush_operation = operation
        self._set_cursor()

    def set_fill_value(self, fill_value: int) -> None:
        self.fill_value = fill_value

    def set_matrix(self, matrix: Optional[np.ndarray]) -> None:
        self.matrix = matrix

    def OnBIEnterInteractor(self, obj: Any, evt: Any) -> None:
    ...
    def OnBILeaveInteractor(self, obj: Any, evt: Any) -> None:
        self.viewer.slice_data.cursor.Show(0)
        self.viewer.interactor.SetCursor(wx.Cursor(wx.CURSOR_DEFAULT))
        self.viewer.interactor.Render()


    def OnBIBrushClick(self, obj: object, evt: object) -> None:
        try:
        ...
        viewer: object = self.viewer
        iren: object = viewer.interactor
        operation: object = self.config.operation
        ...
        cursor: object = slice_data.cursor
        radius: int = cursor.radius
    ...
    def OnBIBrushMove(self, obj: object, evt: object) -> None:
        try:
        ...
        if (self.left_pressed):
            ...
            position: int = viewer.get_slice_pixel_coord_by_world_pos(wx, wy, wz)
            index: int = slice_data.number
    ...
    def OnBIBrushRelease(self, evt: object, obj: object) -> None:
        try:
    ...
    def edit_mask_pixel(self, fill_value: int, n: int, index: object, position: int, radius: int, orientation: str) -> None:
        if orientation == 'AXIAL':
            matrix: object = self.matrix[n, :, :]
        elif orientation == 'CORONAL':
            matrix: object = self.matrix[:, n, :]
        elif orientation == 'SAGITAL':
            matrix: object = self.matrix[:, :, n]

        spacing: object = self.viewer.slice_.spacing
        if hasattr(position, '__iter__'):
            ...
            if orientation == 'AXIAL':
                sx: float = spacing[0]
                sy: float = spacing[1]
                py: int = position / matrix.shape[1]
                px: int = position % matrix.shape[1]
            elif orientation == 'CORONAL':
                sx: float = spacing[0]
                sy: float = spacing[2]
                py: int = position / matrix.shape[1]
        ...
        cx: int = index.shape[1] / 2 + 1
        cy: int = index.shape[0] / 2 + 1
        xi: int = int(px - index.shape[1] + cx)
        xf: int = int(xi + index.shape[1])
        yi: int = int(py - index.shape[0] + cy)
        yf: int = int(yi + index.shape[0])

        if yi < 0:
            index: object = index[abs(yi):,:]
            yi: int = 0
        if yf > matrix.shape[0]:
            index: object = index[:index.shape[0]-(yf-matrix.shape[0]), :]
            yf: int = matrix.shape[0]

        if xi < 0:
            index: object = index[:,abs(xi):]
            xi: int = 0
        if xf > matrix.shape[1]:
            index: object = index[:,:index.shape[1]-(xf-matrix.shape[1])]
            xf: int = matrix.shape[1]
        ...
        roi_m: object = matrix[yi:yf,xi:xf]

        # Checking if roi_i has at least one element.
        if roi_m.size:
            roi_m[index] = self.fill_value

    def OnBIScrollForward(self, evt: Any, obj: Any) -> None:
        iren: vtkRenderWindowInteractor = self.viewer.interactor
        if iren.GetControlKey():
            size: int = self.brush_size + 1
            if size <= 100:
                self.set_brush_size(size)
        else:
            self.OnScrollForward(obj, evt)

    def OnBIScrollBackward(self, evt: Any, obj: Any) -> None:
        iren: vtkRenderWindowInteractor = self.viewer.interactor
        if iren.GetControlKey():
            size: int = self.brush_size - 1
            if size > 0:
...
class CrossInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Any) -> None:
        ...
        self.viewer: Any = viewer
        self.orientation: str = viewer.orientation
        self.slice_actor: vtkActor = viewer.slice_data.actor
        self.slice_data: Any = viewer.slice_data
    ...
    def OnCrossMouseClick(self, obj: Any, evt: Any) -> None:
        iren: vtkRenderWindowInteractor = obj.GetInteractor()
        self.ChangeCrossPosition(iren)

    def OnCrossMove(self, obj: Any, evt: Any) -> None:
    ...
    def OnScrollBar(self, *args: Any, **kwargs: Any) -> None:
            # Update other slice's cross according to the new focal point from
            # the actual orientation.
            x: float
            y: float
            z: float = self.viewer.cross.GetFocalPoint()
            self.viewer.UpdateSlicesPosition([x, y, z])
            Publisher.sendMessage('Set cross focal point', position=[x, y, z, None, None, None])
            Publisher.sendMessage('Update slice viewer')
...
class TractsInteractorStyle(CrossInteractorStyle):
    ...
    def OnTractsMove(self, obj: Any, evt: Any) -> None:
        # The user moved the mouse with left button pressed
        if self.left_pressed:
    ...
    def OnTractsMouseClick(self, obj: Any, evt: Any) -> None:
        # print("Single mouse click")
        # self.tracts = dtr.compute_and_visualize_tracts(self.tracker, self.seed, self.left_pressed)
        self.ChangeTracts(True)

    def OnTractsReleaseLeftButton(self, obj: Any, evt: Any) -> None:
        # time.sleep(3.)
        self.tracts.stop()
        # self.ChangeCrossPosition(iren)


    def ChangeTracts(self, pressed: bool) -> None:
...
class WWWLInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Viewer) -> None:
        ...
        self.acum_achange_window: float = viewer.slice_.window_width
        self.acum_achange_level: float = viewer.slice_.window_level
    ...
    def OnWindowLevelMove(self, obj: Any, evt: Any) -> None:
        if (self.left_pressed):
            iren = obj.GetInteractor()
            mouse_x, mouse_y = self.GetMousePosition()
...
class LinearMeasureInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Viewer) -> None:
        ...
        self.viewer: Viewer = viewer
        self.orientation: str = viewer.orientation
        self.slice_data: SliceData = viewer.slice_data

        self.measures: MeasureData = MeasureData()
        self.selected: Optional[Union[int, Tuple[int, Measure, MeasureRepresentation]]] = None
        self.creating: Optional[Tuple[int, Measure, MeasureRepresentation]] = None

        self._type: str = const.LINEAR

        spacing: Tuple[float, float, float] = self.slice_data.actor.GetInput().GetSpacing()

        if self.orientation == "AXIAL":
            self.radius: float = min(spacing[1], spacing[2]) * 0.8
            self._ori: str = const.AXIAL

        elif self.orientation == 'CORONAL':
            self.radius: float = min(spacing[0], spacing[1]) * 0.8
            self._ori: str = const.CORONAL

        elif self.orientation == 'SAGITAL':
            self.radius: float = min(spacing[1], spacing[2]) * 0.8
            self._ori: str = const.SAGITAL
    ...
    def OnInsertMeasurePoint(self, obj, evt) -> None:
        ...
        selected: Optional[Union[int, Tuple[int, Measure, MeasureRepresentation]]] =  self._verify_clicked_display(mx, my)
        if selected:
            self.selected = selected
            self.viewer.scroll_enabled = False
        else:
            if self.picker.GetViewProp():
                renderer: vtkRenderer = self.viewer.slice_data.renderer
                Publisher.sendMessage("Add measurement point",
                                      position=(x, y, z), type=self._type,
                                      location=ORIENTATIONS[self.orientation],
                ...
                n: int = 1
                m: Measure = self.measures.measures[self._ori][slice_number][-1][0]
                mr: MeasureRepresentation = self.measures.measures[self._ori][slice_number][-1][1]
                self.creating = n, m, mr
                self.viewer.UpdateCanvas()
                self.viewer.scroll_enabled = False


    def OnReleaseMeasurePoint(self, obj: Any, evt: Any) -> None:
        if self.selected:
            n: int
            m: Measure
            mr: MeasureRepresentation
            n, m, mr = self.selected
            x: float
            y: float
            z: float = self._get_pos_clicked()
    ...
    def OnMoveMeasurePoint(self, obj: Any, evt: Any) -> None:
        x: float
        y: float
        z: float = self._get_pos_clicked()
        if self.selected:
    ...
    def _get_pos_clicked(self) -> Tuple[float, float, float]:
        ...
        x: float
        y: float
        z: float = self.GetPickPosition()
        self.picker.DeletePickList(self.slice_data.actor)
        return (x, y, z)

    def _verify_clicked(self, x: float, y: float, z: float) -> Optional[Tuple[int, Measure, MeasureRepresentation]]:
        slice_number: int = self.slice_data.number
        sx: float
        sy: float
        sz: float = self.viewer.slice_.spacing
        if self.orientation == "AXIAL":
            max_dist: float = 2 * max(sx, sy)
        elif self.orientation == "CORONAL":
            max_dist: float = 2 * max(sx, sz)
        elif self.orientation == "SAGITAL":
            max_dist: float = 2 * max(sy, sz)
            ...
            for m, mr in self.measures.measures[self._ori][slice_number]:
                if mr.IsComplete():
                    for n, p in enumerate(m.points):
                        ...
                        py: float
                        pz: float = p
                        dist: float = ((px-x)**2 + (py-y)**2 + (pz-z)**2)**0.5
                        if dist < max_dist:
                            return (n, m, mr)
        return None
    

    def _verify_clicked_display(self, x: float, y: float, max_dist: float = 5.0) -> Optional[Tuple[int, Measure, MeasureRepresentation]]:
            slice_number: int = self.slice_data.number
                ...
                for m, mr in self.measures.measures[self._ori][slice_number]:
                    if mr.IsComplete():
                        for n, p in enumerate(m.points):
                            coord.SetValue(p)
                            cx: float
                            cy: float = coord.GetComputedDisplayValue(self.viewer.slice_data.renderer)
...
class AngularMeasureInteractorStyle(LinearMeasureInteractorStyle):
    def __init__(self, viewer: Any) -> None:
        ...
        self._type: str = const.ANGULAR

        self.state_code: int = const.STATE_MEASURE_ANGLE
...
class DensityMeasureStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: int = const.STATE_MEASURE_DENSITY

        self.format: str = 'polygon'

        self._last_measure: Optional[MeasureData] = None

        self.viewer: Any = viewer
        self.orientation: Any = viewer.orientation
        self.slice_data: Any = viewer.slice_data
    ...
    def _2d_to_3d(self, pos: Tuple[int, int]) -> Tuple[float, float, float]:
        mx, my = pos
        iren = self.viewer.interactor
        render = iren.FindPokedRenderer(mx, my)
        self.picker.AddPickList(self.slice_data.actor)
    ...
    def OnInsertPoint(self, evt: Any) -> None:
    ...
    def OnInsertPolygon(self, evt: Any) -> None:
...
class DensityMeasureEllipseStyle(DensityMeasureStyle):
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: str = const.STATE_MEASURE_DENSITY_ELLIPSE
...
class DensityMeasurePolygonStyle(DensityMeasureStyle):
    def __init__(self, viewer: Any) -> None:
        DensityMeasureStyle.__init__(self, viewer)
        self.state_code: str = const.STATE_MEASURE_DENSITY_POLYGON
        self.format: str = 'polygon'
...
class PanMoveInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: str = const.STATE_PAN
    ...
    def OnPanMove(self, obj: Any, evt: Any) -> None:
        if self.left_pressed:
            obj.Pan()
...
class SpinInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: str = const.STATE_SPIN
...
class ZoomInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: str = const.STATE_ZOOM
    ...
    def OnZoomMoveLeft(self, obj: Any, evt: Any) -> None:
        if self.left_pressed:
            obj.Dolly()
            obj.OnRightButtonDown()
...
class ZoomSLInteractorStyle(vtkInteractorStyleRubberBandZoom):
    ...
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: str = const.STATE_ZOOM_SL
...
class ChangeSliceInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: str = const.SLICE_STATE_SCROLL
    ...
    def OnChangeSliceMove(self, evt: Any, obj: Any) -> None:
        if self.left_pressed:
            min: int = 0
            max: int = self.viewer.slice_.GetMaxSliceNumber(self.viewer.orientation)

            position: Tuple[int, int] = self.viewer.interactor.GetLastEventPosition()
            scroll_position: int = self.viewer.scroll.GetThumbPosition()
    ...
    def OnChangeSliceClick(self, evt: Any, obj: Any) -> None:
        position: Tuple[int, int] = self.viewer.interactor.GetLastEventPosition()
        self.acum_achange_slice: int = self.viewer.scroll.GetThumbPosition()
        self.last_position: int = position[1]


class EditorConfig(metaclass=utils.Singleton):
    def __init__(self) -> None:
        self.operation: str = const.BRUSH_THRESH
        self.cursor_type: str = const.BRUSH_CIRCLE
        self.cursor_size: int = const.BRUSH_SIZE
        self.cursor_unit: str = 'mm'
...
class EditorInteractorStyle(DefaultInteractorStyle):
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: str = const.SLICE_STATE_EDITOR

        self.viewer: Any = viewer
        self.orientation: str = self.viewer.orientation

        self.config: EditorConfig = EditorConfig()
    ...
    def OnEnterInteractor(self, obj: Any, evt: Any) -> None:
        if (self.viewer.slice_.buffer_slices[self.orientation].mask is None):
            return
        self.viewer.slice_data.cursor.Show()
        self.viewer.interactor.SetCursor(wx.Cursor(wx.CURSOR_BLANK))
    ...
    def OnLeaveInteractor(self, obj: Any, evt: Any) -> None:
        self.viewer.slice_data.cursor.Show(0)
        self.viewer.interactor.SetCursor(wx.Cursor(wx.CURSOR_DEFAULT))
        self.viewer.interactor.Render()

    def OnBrushClick(self, obj: Any, evt: Any) -> None:
        if (self.viewer.slice_.buffer_slices[self.orientation].mask is None):
            return
    ...
    def OnBrushMove(self, obj: Any, evt: Any) -> None:
        if (self.viewer.slice_.buffer_slices[self.orientation].mask is None):
            return
    ...
    def OnBrushRelease(self, evt: Any, obj: Any) -> None:
        if (self.viewer.slice_.buffer_slices[self.orientation].mask is None):
            return
    ...
    def EOnScrollForward(self, evt: Any, obj: Any) -> None:
        iren = self.viewer.interactor
        viewer = self.viewer
        if iren.GetControlKey():
            mouse_x, mouse_y = self.GetMousePosition()
    ...
    def EOnScrollBackward(self, evt: Any, obj: Any) -> None:
        iren = self.viewer.interactor
        viewer = self.viewer
        if iren.GetControlKey():
            mouse_x, mouse_y = self.GetMousePosition()
...
class WatershedProgressWindow:
    def __init__(self, process: Any) -> None:
        ...
        self.msg: str = _("Applying watershed ...")
...
class WatershedConfig(metaclass=utils.Singleton):
    def __init__(self) -> None:
        self.algorithm: str = "Watershed"
        self.con_2d: int = 4
        self.con_3d: int = 6
        self.mg_size: int = 3
        self.use_ww_wl: bool = True
        self.operation: str = BRUSH_FOREGROUND
        self.cursor_type: str = const.BRUSH_CIRCLE
        self.cursor_size: int = const.BRUSH_SIZE
        self.cursor_unit: str = 'mm'
    ...
    def set_use_ww_wl(self, use_ww_wl: bool) -> None:
        self.use_ww_wl = use_ww_wl

    def set_algorithm(self, algorithm: str) -> None:
        self.algorithm = algorithm

    def set_2dcon(self, con_2d: int) -> None:
        self.con_2d = con_2d

    def set_3dcon(self, con_3d: int) -> None:
        self.con_3d = con_3d

    def set_gaussian_size(self, size: int) -> None:
        self.mg_size = size

WALGORITHM: Dict[str, Any] = {"Watershed": watershed,
...
class WaterShedInteractorStyle(DefaultInteractorStyle):
    def __init__(self, viewer: Any) -> None:
        ...
        self.state_code: int = const.SLICE_STATE_WATERSHED

        self.viewer: Any = viewer
        self.orientation: str = self.viewer.orientation
        self.matrix: Optional[np.ndarray] = None

        self.config: WatershedConfig = WatershedConfig()
    ...
    def SetUp(self) -> None:
        mask: np.ndarray = self.viewer.slice_.current_mask.matrix
        self._create_mask()
    ...
    def _set_cursor(self) -> None:
        if self.config.cursor_type == const.BRUSH_SQUARE:
            cursor: ca.CursorRectangle = ca.CursorRectangle()
        elif self.config.cursor_type == const.BRUSH_CIRCLE:
            cursor: ca.CursorCircle = ca.CursorCircle()

        cursor.SetOrientation(self.orientation)
        n: int = self.viewer.slice_data.number
        coordinates: Dict[str, List[int]] = {"SAGITAL": [n, 0, 0],
                                                "CORONAL": [0, n, 0],
                                                "AXIAL": [0, 0, n]}
        cursor.SetPosition(coordinates[self.orientation])
        spacing: Tuple[float, float, float] = self.viewer.slice_.spacing
        cursor.SetSpacing(spacing)
        cursor.SetColour(self.viewer._brush_cursor_colour)
        cursor.SetSize(self.config.cursor_size)
        cursor.SetUnit(self.config.cursor_unit)
    ...
    def set_bformat(self, brush_format: str) -> None:
    ...
    def WOnScrollBackward(self, obj: Any, evt: Any) -> None:
        iren: vtkRenderWindowInteractor = self.viewer.interactor
        viewer: Any = self.viewer
        if iren.GetControlKey():
            mouse_x, mouse_y = self.GetMousePosition()
            render = iren.FindPokedRenderer(mouse_x, mouse_y)
            slice_data = self.viewer.get_slice_data(render)
    ...
    def WOnScrollForward(self, obj: Any, evt: Any) -> None:
        iren: vtkRenderWindowInteractor = self.viewer.interactor
        viewer: Any = self.viewer
        if iren.GetControlKey():
            mouse_x, mouse_y = self.GetMousePosition()
    ...
    def OnBrushClick(self, obj: Any, evt: Any) -> None:
        ...
        viewer: Any = self.viewer
        iren: vtkRenderWindowInteractor = viewer.interactor
        ...
        if operation == BRUSH_FOREGROUND:
            if iren.GetControlKey():
                operation: int = BRUSH_BACKGROUND
            elif iren.GetShiftKey():
    ...
    def OnBrushRelease(self, evt: Any, obj: Any) -> None:
        ...
        if self.orientation == 'AXIAL':
            image: np.ndarray = self.viewer.slice_.matrix[n]
            mask: np.ndarray = self.viewer.slice_.current_mask.matrix[n+1, 1:, 1:]
            self.viewer.slice_.current_mask.matrix[n+1, 0, 0] = 1
            markers: np.ndarray = self.matrix[n]

        elif self.orientation == 'CORONAL':
            image: np.ndarray = self.viewer.slice_.matrix[:, n, :]
            mask: np.ndarray = self.viewer.slice_.current_mask.matrix[1:, n+1, 1:]
            self.viewer.slice_.current_mask.matrix[0, n+1, 0]
            markers: np.ndarray = self.matrix[:, n, :]

        elif self.orientation == 'SAGITAL':
            image: np.ndarray = self.viewer.slice_.matrix[:, :, n]
            mask: np.ndarray = self.viewer.slice_.current_mask.matrix[1: , 1:, n+1]
            self.viewer.slice_.current_mask.matrix[0 , 0, n+1]
            markers: np.ndarray = self.matrix[:, :, n]

        ww: float = self.viewer.slice_.window_width
        wl: float = self.viewer.slice_.window_level

        if BRUSH_BACKGROUND in markers and BRUSH_FOREGROUND in markers:
            #w_algorithm = WALGORITHM[self.config.algorithm]
            bstruct: np.ndarray = generate_binary_structure(2, CON2D[self.config.con_2d])
            if self.config.use_ww_wl:
                if self.config.algorithm == 'Watershed':
                    tmp_image: np.ndarray = ndimage.morphological_gradient(
                                    get_LUT_value(image, ww, wl).astype('uint16'),
                                    self.config.mg_size)
                    tmp_mask: np.ndarray = watershed(tmp_image, markers.astype('int16'), bstruct)
                else:
                    ...
                    #tmp_image = ndimage.morphological_gradient(
                                    #get_LUT_value(image, ww, wl).astype('uint16'),
                                    #self.config.mg_size)
                    tmp_image: np.ndarray = get_LUT_value(image, ww, wl).astype('uint16')
                    #markers[markers == 2] = -1
                    tmp_mask: np.ndarray = watershed_ift(tmp_image, markers.astype('int16'), bstruct)
                    #markers[markers == -1] = 2
                    #tmp_mask[tmp_mask == -1]  = 2

            else:
                if self.config.algorithm == 'Watershed':
                    tmp_image: np.ndarray = ndimage.morphological_gradient((image - image.min()).astype('uint16'), self.config.mg_size)
                    tmp_mask: np.ndarray = watershed(tmp_image, markers.astype('int16'), bstruct)
                else:
                    ...
                    tmp_image: np.ndarray = image - image.min().astype('uint16')
                    tmp_mask: np.ndarray = watershed_ift(tmp_image, markers.astype('int16'), bstruct)
    ...
    def edit_mask_pixel(self, operation: int, n: int, index: np.ndarray, position: int or tuple, radius: float, orientation: str) -> None:
        if orientation == 'AXIAL':
            mask = self.matrix[n, :, :]
        elif orientation == 'CORONAL':
            mask = self.matrix[:, n, :]
        ...
        cx: float = index.shape[1] / 2 + 1
        cy: float = index.shape[0] / 2 + 1
    ...
    def expand_watershed(self) -> None:
        markers: np.ndarray = self.matrix
        image: np.ndarray = self.viewer.slice_.matrix
        self.viewer.slice_.do_threshold_to_all_slices()
        mask: np.ndarray = self.viewer.slice_.current_mask.matrix[1:, 1:, 1:]
        ww: int = self.viewer.slice_.window_width
        wl: int = self.viewer.slice_.window_level
        if BRUSH_BACKGROUND in markers and BRUSH_FOREGROUND in markers:
            #w_algorithm = WALGORITHM[self.config.algorithm]
            bstruct: np.ndarray = generate_binary_structure(3, CON3D[self.config.con_3d])
            tfile: str = tempfile.mktemp()
            tmp_mask: np.memmap = np.memmap(tfile, shape=mask.shape, dtype=mask.dtype,
                                        mode='w+')
            q: multiprocessing.Queue = multiprocessing.Queue()
            p: multiprocessing.Process = multiprocessing.Process(target=watershed_process.do_watershed, args=(image,
                                        markers, tfile, tmp_mask.shape, bstruct,
                                        self.config.algorithm,
                                        self.config.mg_size,
                                        self.config.use_ww_wl, wl, ww, q))

            wp: WatershedProgressWindow = WatershedProgressWindow(p)
            p.start()
...
class ReorientImageInteractorStyle(DefaultInteractorStyle):
    ...
    def __init__(self, viewer: object) -> None:
        ...
        self.state_code: int = const.SLICE_STATE_REORIENT

        self.viewer: object = viewer

        self.line1: object = None
        self.line2: object = None

        self.actors: list = []

        self._over_center: bool = False
        self.dragging: bool = False
        self.to_rot: bool = False

        self.picker: object = vtkWorldPointPicker()
    ...
    def CleanUp(self) -> None:
        ...
        self.viewer.slice_.rotations: list = [0, 0, 0]
        self.viewer.slice_.q_orientation: np.array = np.array((1, 0, 0, 0))
        self._discard_buffers()
        Publisher.sendMessage('Close reorient dialog')
        Publisher.sendMessage('Show current mask')

    def OnLeftClick(self, obj: object, evt: object) -> None:
        if self._over_center:
            ...
            self.p0: tuple = self.get_image_point_coord(x, y, z)
            self.to_rot: bool = True

    def OnLeftRelease(self, obj: object, evt: object) -> None:
        self.dragging: bool = False
    ...
    def OnMouseMove(self, obj: object, evt: object) -> None:
        """
        ...
        if self.dragging:
            ...
            center: tuple = self.viewer.slice_.center
            coord: object = vtkCoordinate()
            coord.SetValue(center)
            cx, cy = coord.GetComputedDisplayValue(self.viewer.slice_data.renderer)

            dist_center: float = ((mx - cx)**2 + (my - cy)**2)**0.5
            if dist_center <= 15:
                self._over_center: bool = True
                cursor: object = wx.Cursor(wx.CURSOR_SIZENESW)
            else:
                self._over_center: bool = False
                cursor: object = wx.Cursor(wx.CURSOR_DEFAULT)
    ...
    def OnDblClick(self, evt: object) -> None:
        self.viewer.slice_.rotations: list = [0, 0, 0]
        self.viewer.slice_.q_orientation: np.array = np.array((1, 0, 0, 0))
    ...
    def _rotate(self) -> None:
        ...
        if self.viewer.orientation == 'AXIAL':
            p1: np.ndarray = np.array((y-cy, x-cx))
        elif self.viewer.orientation == 'CORONAL':
            p1: np.ndarray = np.array((z-cz, x-cx))
        elif self.viewer.orientation == 'SAGITAL':
            p1: np.ndarray = np.array((z-cz, y-cy))
        p0: np.ndarray = self.p0
        p1: np.ndarray = self.get_image_point_coord(x, y, z)

        axis: np.ndarray = np.cross(p0, p1)
        norm: float = np.linalg.norm(axis)
        if norm == 0:
            return
        axis = axis / norm
        angle: float = np.arccos(np.dot(p0, p1)/(np.linalg.norm(p0)*np.linalg.norm(p1)))
    ...
    def get_image_point_coord(self, x: float, y: float, z: float) -> np.ndarray:
        cx: float
        cy: float
        cz: float
        cx, cy, cz = self.viewer.slice_.center
        ...
        M: np.ndarray = transformations.quaternion_matrix(self.viewer.slice_.q_orientation)
        tcoord: np.ndarray = np.array((z, y, x, 1)).dot(M)
        tcoord = tcoord[:3]/tcoord[3]

        #  print (z, y, x), tcoord
        return tcoord

    def _set_reorientation_angles(self, angles: Tuple[float, float, float]) -> None:
        ax: float
        ay: float
        az: float
        ax, ay, az = angles
        q: np.ndarray = transformations.quaternion_from_euler(az, ay, ax)
    ...
    def _create_line(self, x0: float, y0: float, x1: float, y1: float, color: Tuple[float, float, float]) -> vtkLineSource:
        line: vtkLineSource = vtkLineSource()
        ...
        mapper: vtkPolyDataMapper2D = vtkPolyDataMapper2D()
...
class FloodFillMaskInteractorStyle(DefaultInteractorStyle):
    def __init__(self, viewer: object) -> None:
        ...
        self.state_code: int = const.SLICE_STATE_MASK_FFILL

        self.viewer: object = viewer
        self.orientation: str = self.viewer.orientation

        self.picker: vtkWorldPointPicker = vtkWorldPointPicker()
        self.slice_actor: object = viewer.slice_data.actor
        self.slice_data: object = viewer.slice_data

        self.config: FFillConfig = FFillConfig()
        self.dlg_ffill: object = None
        ...
        self._dlg_title: str = _(u"Fill holes")
        self._progr_title: str = _(u"Fill hole")
        self._progr_msg: str = _(u"Filling hole ...")

        self.AddObserver("LeftButtonPressEvent", self.OnFFClick)

    def SetUp(self) -> None:
        if not self.config.dlg_visible:
            self.config.dlg_visible: bool = True
            self.dlg_ffill: object = dialogs.FFillOptionsDialog(self._dlg_title, self.config)
            self.dlg_ffill.Show()

    def CleanUp(self) -> None:
        if (self.dlg_ffill is not None) and (self.config.dlg_visible):
            self.config.dlg_visible: bool = False
            self.dlg_ffill.Destroy()
            self.dlg_ffill: object = None

    def OnFFClick(self, obj: object, evt: object) -> None:
        if (self.viewer.slice_.buffer_slices[self.orientation].mask is None):
            return
        ...
        if self.config.target == "3D":
            bstruct: np.ndarray = np.array(generate_binary_structure(3, CON3D[self.config.con_3d]), dtype='uint8')
            self.viewer.slice_.do_threshold_to_all_slices()
            cp_mask: np.ndarray = self.viewer.slice_.current_mask.matrix.copy()
        else:
            _bstruct: np.ndarray = generate_binary_structure(2, CON2D[self.config.con_2d])
            if self.orientation == 'AXIAL':
                bstruct: np.ndarray = np.zeros((1, 3, 3), dtype='uint8')
                bstruct[0] = _bstruct
            elif self.orientation == 'CORONAL':
                bstruct: np.ndarray = np.zeros((3, 1, 3), dtype='uint8')
                bstruct[:, 0, :] = _bstruct
            elif self.orientation == 'SAGITAL':
                bstruct: np.ndarray = np.zeros((3, 3, 1), dtype='uint8')
                bstruct[:, :, 0] = _bstruct

        if self.config.target == '2D':
            floodfill.floodfill_threshold(mask, [[x, y, z]], self.t0, self.t1, self.fill_value, bstruct, mask)
            b_mask: np.ndarray = self.viewer.slice_.buffer_slices[self.orientation].mask
            index: int = self.viewer.slice_.buffer_slices[self.orientation].index

            if self.orientation == 'AXIAL':
                p_mask: np.ndarray = mask[index,:,:].copy()
            elif self.orientation == 'CORONAL':
                p_mask: np.ndarray = mask[:, index, :].copy()
            elif self.orientation == 'SAGITAL':
                p_mask: np.ndarray = mask[:, :, index].copy()
            ...
            with futures.ThreadPoolExecutor(max_workers=1) as executor:
                future: object = executor.submit(floodfill.floodfill_threshold, mask, [[x, y, z]], self.t0, self.t1, self.fill_value, bstruct, mask)

                dlg: wx.ProgressDialog = wx.ProgressDialog(self._progr_title, self._progr_msg, parent=wx.GetApp().GetTopWindow(), style=wx.PD_APP_MODAL)
        ...
        self.viewer.slice_.current_mask.was_edited: bool = True
        self.viewer.slice_.current_mask.modified(True)
        Publisher.sendMessage('Reload actual slice')
...
class RemoveMaskPartsInteractorStyle(FloodFillMaskInteractorStyle):
    def __init__(self, viewer: object) -> None:
        ...
        self.state_code: int = const.SLICE_STATE_REMOVE_MASK_PARTS
        ...
        self._dlg_title: str = _(u"Remove parts")
        self._progr_title: str = _(u"Remove part")
        self._progr_msg: str = _(u"Removing part ...")
...
class CropMaskInteractorStyle(DefaultInteractorStyle):
    def __init__(self, viewer: object) -> None:
        ...
        self.state_code: int = const.SLICE_STATE_CROP_MASK
        ...
        self.slice_actor: vtkActor = viewer.slice_data.actor
        self.slice_data: SliceData = viewer.slice_data
        self.draw_retangle: geom.DrawCrop2DRetangle = None

        self.config: CropMaskConfig = CropMaskConfig()
    ...
    def OnReleaseLeftButton(self, obj: object, evt: object) -> None:
        self.draw_retangle.mouse_pressed: bool = False
        self.draw_retangle.ReleaseLeft()

    def SetUp(self) -> None:
        self.draw_retangle: geom.DrawCrop2DRetangle = geom.DrawCrop2DRetangle()
        ...
        if not(self.config.dlg_visible):
            ...
            dlg: dialogs.CropOptionsDialog = dialogs.CropOptionsDialog(self.config)
    ...
    def CropMask(self) -> None:
        if self.viewer.orientation == "AXIAL":
            ...
            tmp_mask: np.ndarray = self.viewer.slice_.current_mask.matrix[zi-1:zf+1, yi-1:yf+1, xi-1:xf+1].copy()
...
class SelectMaskPartsInteractorStyle(DefaultInteractorStyle):
    def __init__(self, viewer: Viewer) -> None:
        ...
        self.state_code: int = const.SLICE_STATE_SELECT_MASK_PARTS

        self.viewer: Viewer = viewer
        self.orientation: int = self.viewer.orientation

        self.picker: vtkWorldPointPicker = vtkWorldPointPicker()
        self.slice_actor: vtkActor = viewer.slice_data.actor
        self.slice_data: SliceData = viewer.slice_data

        self.config: SelectPartConfig = SelectPartConfig()
        self.dlg: dialogs.SelectPartsOptionsDialog = None
    ...
    def SetUp(self) -> None:
        if not self.config.dlg_visible:
            ...
            default_name: str =  const.MASK_NAME_PATTERN %(mask.Mask.general_index+2)

            self.config.mask_name: str = default_name
            self.config.dlg_visible: bool = True
            self.dlg: dialogs.SelectPartsOptionsDialog = dialogs.SelectPartsOptionsDialog(self.config)
            self.dlg.Show()
    ...
    def CleanUp(self) -> None:
        ...
        dialog_return: int = self.dlg.GetReturnCode()
        ...
        if self.config.mask:
            if dialog_return == wx.OK:
                self.config.mask.name: str = self.config.mask_name
            ...
            self.viewer.slice_.to_show_aux: str = ''
            Publisher.sendMessage('Reload actual slice')
            self.config.mask: Mask = None
    ...
    def OnSelect(self, obj, evt) -> None:
        ...
        self.viewer.slice_.aux_matrices['SELECT']: np.ndarray = self.config.mask.matrix[1:, 1:, 1:]
        self.viewer.slice_.to_show_aux: str = 'SELECT'

        self.config.mask.was_edited: bool = True
        Publisher.sendMessage('Reload actual slice')

    def _create_new_mask(self) -> None:
        mask: Mask = self.viewer.slice_.create_new_mask(show=False, add_to_project=False)
        mask.was_edited: bool = True
        mask.matrix[0, :, :] = 1
        mask.matrix[:, 0, :] = 1
        mask.matrix[:, :, 0] = 1

        self.config.mask: Mask = mask


class FFillSegmentationConfig(metaclass=utils.Singleton):
    def __init__(self) -> None:
        self.dlg_visible: bool = False
        self.dlg: dialogs.FFillSegmentationOptionsDialog = None
        self.target: str = "2D"
        self.con_2d: int = 4
        self.con_3d: int = 6
        ...
        self.confid_mult: float = 2.5
        self.confid_iters: int =3


class FloodFillSegmentInteractorStyle(DefaultInteractorStyle):
    def __init__(self, viewer: vtkImageViewer2) -> None:
        DefaultInteractorStyle.__init__(self, viewer)

        self.state_code: int = const.SLICE_STATE_FFILL_SEGMENTATION

        self.viewer: vtkImageViewer2 = viewer
        self.orientation: str = self.viewer.orientation

        self.picker: vtkWorldPointPicker = vtkWorldPointPicker()
        self.slice_actor: vtkImageActor = viewer.slice_data.actor
        self.slice_data: SliceData = viewer.slice_data

        self.config: FFillSegmentationConfig = FFillSegmentationConfig()

        self._progr_title: str = _(u"Region growing")
        self._progr_msg: str = _(u"Segmenting ...")
    ...
    def SetUp(self) -> None:
        if not self.config.dlg_visible:
            ...
            if self.config.t0 is None:
                image: np.ndarray = self.viewer.slice_.matrix
                _min: float = image.min()
                _max: float = image.max()

                self.config.t0: int = int(_min + (3.0/4.0) * (_max - _min))
                self.config.t1: int = int(_max)

            self.config.dlg_visible: bool = True
            self.config.dlg: dialogs.FFillSegmentationOptionsDialog = dialogs.FFillSegmentationOptionsDialog(self.config)
    ...
    def CleanUp(self) -> None:
        if (self.config.dlg is not None) and (self.config.dlg_visible):
            ...
            self.config.dlg: Optional[dialogs.FFillSegmentationOptionsDialog] = None

    def OnFFClick(self, obj: Any, evt: Any) -> None:
        if (self.viewer.slice_.buffer_slices[self.orientation].mask is None):
            return
    ...
    def do_2d_seg(self) -> None:
        ...
        mask: np.ndarray = self.viewer.slice_.buffer_slices[self.orientation].mask.copy()
        image: np.ndarray = self.viewer.slice_.buffer_slices[self.orientation].image
        ...
        if self.config.method == 'confidence':
            ...
            bstruct: np.ndarray = np.array(generate_binary_structure(2, CON2D[self.config.con_2d]), dtype='uint8')
            bstruct: np.ndarray = bstruct.reshape((1, 3, 3))

            out_mask: np.ndarray = self.do_rg_confidence(image, mask, (x, y, 0), bstruct)
        else:
            if self.config.method == 'threshold':
                v: float = image[y, x]
                t0: int = self.config.t0
                t1: int = self.config.t1
                ...
                if self.config.use_ww_wl:
                    ...
                    ww: float = self.viewer.slice_.window_width
                    wl: float = self.viewer.slice_.window_level
                    image: np.ndarray = get_LUT_value_255(image, ww, wl)

                v: float = image[y, x]

                t0: float = v - self.config.dev_min
                t1: float = v + self.config.dev_max
            ...
            out_mask: np.ndarray = np.zeros_like(mask)
        ...
        mask[out_mask.astype('bool')]: int = self.config.fill_value

        index: int = self.viewer.slice_.buffer_slices[self.orientation].index
        b_mask: np.ndarray = self.viewer.slice_.buffer_slices[self.orientation].mask
        vol_mask: np.ndarray = self.viewer.slice_.current_mask.matrix[1:, 1:, 1:]
    ...
    def do_3d_seg(self) -> None:
        ...
        if self.config.method != 'confidence':
            if self.config.method == 'threshold':
                ...
                t0: int = v - self.config.dev_min
                t1: int = v + self.config.dev_max
        ...
        if self.config.method == 'confidence':
            with futures.ThreadPoolExecutor(max_workers=1) as executor:
                future: object = executor.submit(self.do_rg_confidence, image, mask, (x, y, z), bstruct)
                ...
                out_mask: np.ndarray = future.result()
        else:
            out_mask: np.ndarray = np.zeros_like(mask)
            with futures.ThreadPoolExecutor(max_workers=1) as executor:
                future: object = executor.submit(floodfill.floodfill_threshold, image, [[x, y, z]], t0, t1, 1, bstruct, out_mask)
    ...
    def do_rg_confidence(self, image, mask, p, bstruct):
        ...
        if self.config.use_ww_wl:
            ...
            wl: int = self.viewer.slice_.window_level
            image: np.ndarray = get_LUT_value_255(image, ww, wl)
        bool_mask: np.ndarray = np.zeros_like(mask, dtype='bool')
        out_mask: np.ndarray = np.zeros_like(mask)
        ...
        for i in range(self.config.confid_iters):
            var: float = np.std(image[bool_mask])
            mean: float = np.mean(image[bool_mask])

            t0: int = mean - var * self.config.confid_mult
            t1: int = mean + var * self.config.confid_mult
...
class Styles:
    styles: Dict[int, Type[InteractorStyle]] = {
    ...
    @classmethod
    def add_style(cls, style_cls: Type[InteractorStyle], level: int = 1) -> int:
    ...
    @classmethod
    def get_style(cls, style: int) -> Type[InteractorStyle]:
