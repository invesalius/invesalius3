    ...
    def run(self):
        ...
        while not self.event.is_set():
            try:
                ...
                probe_coord, probe_m_img = corregistrate_probe(coreg_data[1], coreg_data[0], coord_raw, self.ref_mode_id)
                coil_coord, coil_m_img = corregistrate_object_dynamic(coreg_data[1:], coord_raw, self.ref_mode_id, [self.use_icp, self.m_icp])
                
                #m_change = coreg_data[1], r_stylus = coreg_data[0] (r_stylus used for probe only)
                coord_probe, m_img_probe = corregistrate_probe(coreg_data[1], coreg_data[0], coord_raw, self.ref_mode_id)
                coord_coil, m_img_coil = corregistrate_object_dynamic(coreg_data[1:], coord_raw, self.ref_mode_id, [self.use_icp, self.m_icp])
