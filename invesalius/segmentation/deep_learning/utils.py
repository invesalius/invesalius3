...
def prepare_plaidml() -> None:
    # Linux if installed plaidml with pip3 install --user
    if sys.platform.startswith("linux"):
        local_user_plaidml: pathlib.Path = pathlib.Path("~/.local/share/plaidml/").expanduser().absolute()
    ...
    # Mac if using python3 from homebrew
        ...
        local_user_plaidml: pathlib.Path = pathlib.Path("/usr/local/share/plaidml")
        if local_user_plaidml.exists():
            os.environ["RUNFILES_DIR"] = str(local_user_plaidml)
            os.environ["PLAIDML_NATIVE_PATH"] = str(pathlib.Path("/usr/local/lib/libplaidml.dylib").expanduser().absolute())
    elif sys.platform == "win32":
        if 'VIRTUAL_ENV' in os.environ:
            local_user_plaidml: pathlib.Path = pathlib.Path(os.environ["VIRTUAL_ENV"]).joinpath("share/plaidml")
            plaidml_dll: pathlib.Path = pathlib.Path(os.environ["VIRTUAL_ENV"]).joinpath("library/bin/plaidml.dll")
            if local_user_plaidml.exists():
                os.environ["RUNFILES_DIR"] = str(local_user_plaidml)
            if plaidml_dll.exists():
                os.environ["PLAIDML_NATIVE_PATH"] = str(plaidml_dll)

def prepare_ambient(backend: str, device_id: str, use_gpu: bool) -> None:
...
def get_plaidml_devices(gpu: bool = False) -> Dict[str, str]:
    prepare_plaidml()
