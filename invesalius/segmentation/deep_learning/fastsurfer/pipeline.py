...
from .utils import Conformer, Mapper, PreparePlaneView, Validator
...
class Pipeline:
    ...
    def run_pipeline(
        ...
        lut = self.mapper.read_lut(lut_path)
        labels = lut["ID"].values

        prepared_data = self.prepare_input(input_img_path)
        segmentation = self.run_inference(prepared_data, backend, use_gpu, device_id, model_paths)

        segmentation = self.conformer.back_to_orig(segmentation)

        final_segmentation = self.mapper.aparc_aseg_to_label(segmentation, labels)

        final_segmentation = self.mapper.split_cortex_labels(final_segmentation)
