...
def buffer_parse(onnx_tensor: TensorProto) -> Tensor:
    if onnx_tensor.string_data:
        raise NotImplementedError("Parsing for buffer with string data is not implemented.")
    ...
    if onnx_tensor.HasField("raw_data"):
        np_buffer = (
            np.frombuffer(
                onnx_tensor.raw_data, dtype=helper.tensor_dtype_to_np_dtype(onnx_tensor.data_type)
            )
            .copy()
            .reshape(shape)
        )
...
class OnnxRunner:
    def __init__(self, model: ModelProto):
        ...
        self.is_training = any(
            n.domain in {"ai.onnx.training", "ai.onnx.preview.training"} for n in model.graph.node
        )
        ...
        self.graph_inputs = {
            x.name: type_parse(x.type) for x in model.graph.input if x.name not in self.graph_values
        }
    ...
    def __call__(self, inputs, debug=debug):
...
def get_onnx_ops():
    ...
    def _prepare_quantize(x, scale, zero_point, axis=1, block_size=0):
        ...
        if not isinstance(zero_point, Tensor):
            zero_point = Tensor(zero_point, dtype=dtypes.uint8)._broadcast_to(scale.shape)
        if block_size == 0:
            shape = (*[1] * axis, *scale.shape, *[1] * (x.ndim - axis - scale.ndim))
            return scale.reshape(shape), zero_point.reshape(shape)
        return scale.repeat_interleave(block_size, dim=axis), zero_point.repeat_interleave(
            block_size, dim=axis
        )
    ...
    def ImageDecoder(encoded_stream: bytes, pixel_format="RGB"):
        ...
        if pixel_format == "Grayscale":
            return Tensor(np.array(img.convert("L"))).unsqueeze(-1)  # (H, W) to (H, W, 1)
        raise ValueError(f"pixel_format={pixel_format!r} is not supported.")

    def EyeLike(x: Tensor, dtype: int | None = None, k: int = 0):
        ret = Tensor.eye(
            cast(int, min(x.shape)), dtype=dtype_parse(dtype) if dtype is not None else x.dtype
        )
        return (
            ret
    ...
    def Squeeze(data: Tensor, axes: list[int] | None = None):
        return (
            data.squeeze()
            if axes is None
            else functools.reduce(lambda d, dim: d.squeeze(dim), sorted(axes, reverse=True), data)
        )
    ...
    def Split(data: Tensor, split: list[int] | None = None, num_outputs: int = 0, axis: int = 0):
        ...
        if split is None:
            split = [
                sz // num_outputs + (1 if i < sz % num_outputs else 0) for i in range(num_outputs)
            ]
    ...
    def ConvTranspose(
        ...
        strides, dilations, output_padding = (
            make_tuple(x, len(input_shape)) for x in (strides, dilations, output_padding)
        )
            ...
            pads = _auto_pad(
                [
                    ...
                    for s, i, op, k, d, os in zip(
                        strides, input_shape, output_padding, kernel_shape, dilations, output_shape
                    )
                ],
                auto_pad,
            )
    ...
    def MaxUnpool(
        ...
        out_sh = [
            (ks // 2) * 2 + st * inps for inps, st, ks in zip(xI.shape, strides, kernel_shape)
        ]
        ret = (
        ...
        if outshape is not None and outshape != ret.shape:
            pads = _auto_pad(
                [outshape[-2] - ret.shape[-2], outshape[-1] - ret.shape[-1]], "SAME_UPPER"
            )
    ...
    def Resize(
        ...
        def _apply_transformation(index: Tensor, input_dim, scale_dim, roi_dim, mode):
            ...
            if mode == "half_pixel":
                ...
                index = (
                    index * (input_dim - 1) / (output_dim - 1) if output_dim != 1 else Tensor([0])
                )
            elif mode == "asymmetric":
                index = index / scale_dim
            elif mode == "pytorch_half_pixel":
                index = (index + 0.5) / scale_dim - 0.5 if output_dim != 1 else Tensor([-0.5])
        ...
        if sizes is not None:
            if keep_aspect_ratio_policy in ["not_larger", "not_smaller"]:
                ...
                sizes = [
                    int((scales[0] * input_shape[i]) + 0.5) if i + 2 in axes else input_shape[i]
        ...
        for shape, size, scale, region in zip(input_shape, sizes, scales, regions):
            indexes.append(
                _apply_transformation(
                    Tensor.arange(size), shape, scale, region, coordinate_transformation_mode
                )
            )
    ...
    def EmbedLayerNormalization(
        ...
        def embedding(x: Tensor, vocab_size, weight: Tensor) -> Tensor:
            return (
                x.unsqueeze(-1).expand(*x.shape, vocab_size)._one_hot_along_dim(vocab_size) @ weight
            )
        ...
        seg_embedding_res = (
            embedding(segment_ids, type_vocab_size, segment_embedding) if compute_seg_emb else None
        )
    ...
    def Dropout_7(
        data: Tensor, ratio: float = 0.5, training_mode: bool = False, seed: int | None = None
    ):
        ...
        mask = Tensor(
            np.random.RandomState(seed).random(cast(tuple[int, ...], data.shape)) >= ratio,
            requires_grad=False,
            device=data.device,
        )
    ...
    def AffineGrid(theta: Tensor, size: list[int], align_corners: int = 0):
        ...
        def generate_grid(steps):
            return (
                ...
                if align_corners
                else Tensor.linspace(-1 + 1 / steps, 1 - 1 / steps, steps, device=theta.device)
            )
    ...
    def Attention(
        ...
        hidden_size, v_hidden_size = (
            qkv_hidden_sizes[1:] if qkv_hidden_sizes is not None else 2 * (weights.shape[1] // 3,)
        )
        ...
        xq, xk, xv = [
            x.reshape(x.shape[0], x.shape[1], num_heads, -1).transpose(1, 2) for x in (xq, xk, xv)
        ]
        ...
        def attn(query, key, value, attn_mask):
            ...
            causal_mask = Tensor.ones((cdim, cdim), requires_grad=False, dtype=dtypes.bool).tril(0)[
    ...
    def Gather(x: Tensor, indices: Tensor, axis: int = 0):
        if (
            ...
            indices = (
                ...
                if indices.shape == ()
                else [x_sh[axis] + x if x < 0 else x for x in _cached_to_python_const(indices)]
            )
            args = [
                [(0, x) if j != axis else (i, i + 1) for j, x in enumerate(x_sh)] for i in indices
            ]  # type: ignore
            return (
    ...
    def MatMulInteger(
        A: Tensor, B: Tensor, a_zero_point: Tensor | int = 0, b_zero_point: Tensor | int = 0
    ) -> Tensor:
