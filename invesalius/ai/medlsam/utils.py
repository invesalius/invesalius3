...
class ResizeLongestSide:
    ...
    def apply_image(self, image: np.ndarray) -> np.ndarray:
        ...
        """
        # Rewrite to use only torch or numpy, avoiding torchvision
        # Convert to tensor BCHW (1, C, H, W)
        # Image is HxWxC
        h, w, c = image.shape
        ...
        # Interpolate
        # Note: 'bilinear' expects float input
        resized_tensor = F.interpolate(
            img_tensor, size=target_size, mode='bilinear', align_corners=False, antialias=False
        )
        
        # Convert back to HxWxC uint8
