...
def get_svn_revision(path: str = None) -> str:
    """
    ...
    try:
        ...
        if re.match('(\d+)', entries):
            rev_match: re.Match = re.search('\d+\s+dir\s+(\d+)', entries)
            if rev_match:
                rev: str = rev_match.groups()[0]
        # Older XML versions of the file specify revision as an attribute of
        # the first entries node.
        else:
            from xml.dom import minidom
            dom: minidom.Document = minidom.parse(entries_path)
            rev: str = dom.getElementsByTagName('entry')[0].getAttribute('revision')
